...
const Extrailer = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const trailerValues = response.data.records.map(entry => entry.Trailer);
      ...
      moveDateValues.forEach((date, index) => {
        if (date > currentDate &&( trailerValues[index] === 'yes' || trailerValues[index] === 'Yes')) {
