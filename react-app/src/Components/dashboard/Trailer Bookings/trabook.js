...
import "../My Bookings/mybook.css"; // Import the CSS file for styling
...
const Trabooking = () => {
  ...
  const isDateInFuture = (dateStr) => {
    const [day, month, year] = dateStr.split("/");
    const moveDate = new Date(`${year}-${month}-${day}`);
    const currentDate = new Date();
    return moveDate >= currentDate;
  };
  ...
  .filter(booking => booking['Trailer'] === 'yes' || booking['Trailer'] === 'Yes')
  .filter(booking => searchBookingData(booking) && isDateInFuture(booking.MoveDate)
  );
